fetch("http://localhost:3000/getPosts", {
 ...
 container = document.createElement("div");
    ...
    posts.forEach((post) => {
      ...
      // Check if post exists and if post has a userEmail property
      if (post && post.userEmail) {
        let authorName = document.createElement("p");
        authorName.innerText = `Posted By ${post.userEmail}`;
        authorName.style.cssText = `font-size: 10px;`;
        // Assuming authorContainer1 is defined elsewhere in your code
        authorContainer1.appendChild(topic);
        authorContainer1.appendChild(authorName);
        authorContainer1.style.cssText = `display: flex; flex-direction: column; gap: 4px;`;
      }
      // avatar
      let avatarSVG =
      ...
      // author
      let authorName = document.createElement("p");
      authorName.innerText = `Posted By ${post.name}`;
      authorName.style.cssText = `font-size: 10px;`;
      authorContainer1.appendChild(topic);
      authorContainer1.appendChild(authorName);
      authorContainer1.style.cssText = `display: flex; flex-direction: column; gap: 4px;`;
      ...
      commentContainer.style.cssText = `display: flex; flex-direction: row; align-items: center; gap: 8px;`;
      ...
      viewContainer.style.cssText = `display: flex; flex-direction: row; align-items: center; gap: 8px;`;
      ...
      if (pageName === "Home") {
        uploadContainer.style.cssText = `display: flex; flex-direction: row; justify-content: space-between;`;
        uploadContainer.appendChild(commentContainer);
        uploadContainer.appendChild(share);
      } else {
        uploadContainer.appendChild(commentContainer);
        uploadContainer.style.cssText = `display: flex; flex-direction: row; justify-content: flex-end;`;
      }
