...
`;
  ...
  /// Function to check if image exists on the backend
  ...
  function addImage(event) {
    ...
    if (file) {
      ...
      reader.onload = function (e) {
        ...
        img.style.maxHeight = "300px"; // Adjust the maximum height here
        img.style.objectFit = "contain"; // Maintain aspect ratio
        container_img.innerHTML = ""; // Clear previous content
        container_img.appendChild(img);
      };

      reader.readAsDataURL(file);
    } else {
      // Check if image exists on the backend
      const imageUrl = "URL_TO_YOUR_BACKEND_IMAGE"; // Replace with the actual URL to your backend image
      checkImageExists(imageUrl, function (exists) {
        if (exists) {
          // If image exists, create and append the image element
          const img = document.createElement("img");
          img.src = imageUrl;
          img.alt = "Uploaded Image";
          img.style.maxWidth = "100%";
          ...
          // If image does not exist, adjust the space accordingly
          container_img.innerHTML = ""; // Clear previous content
          container_img.style.height = "100px"; // Adjust the height as needed
        }
      });
    }
  }
